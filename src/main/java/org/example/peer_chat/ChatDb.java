...
public class ChatDb {
    ...
    public void insertCallRecord(CallRecord record) {
        String sql = "INSERT INTO call_history (from_user, to_user, timestamp, duration_seconds, accepted) " +
                "VALUES (?, ?, ?, ?, ?)";
        try (Connection c = DriverManager.getConnection(url);
             PreparedStatement p = c.prepareStatement(sql)) {
            p.setString(1, record.getFromUser());
            p.setString(2, record.getToUser());
            p.setLong(3, record.getTimestamp());    // startTs
            p.setLong(4, record.getDuration());     // duration in seconds
            p.setInt(5, record.isSuccess() ? 1 : 0); // success -> accepted
            p.executeUpdate();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<CallRecord> loadCallHistory(String me, String peer, int limit) {
        ...
        String sql = "SELECT from_user, to_user, timestamp, duration_seconds, accepted " +
                "FROM call_history " +
